...
결과 파일도 플롯도 로그도 가져올 수 없다. 운영 실험에서 이쪽으로 돌아오는 것은
...
def version() -> str:
    """VERSION 파일 또는 git 에서 읽는다.

    "어떤 코드로 돌렸는지"는 결과 파일을 가져올 수 없는 상황에서 실행 환경에 남는
...
def peak_memory_gb() -> Optional[float]:
    try:
        ...
        # 실행 환경는 리눅스지만 여기서 시험하면서 40GB 로 찍히면 못 믿게 된다.
...
class Source:
    ...
    값만 찍으면 "왜 저게 저 값이지"를 못 푼다. 실행 환경에서는 .bashrc 의 환경변수,
...
class RunSummary:
    version: str = ""
    # 실행 인자를 그대로 한 줄. 가져올 수 없으므로 "그때 뭘로 돌렸는지"가 셸
