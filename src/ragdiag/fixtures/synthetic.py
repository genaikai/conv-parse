...
CASES = [
    ...
    dict(
        ...
        answer="복지포인트는 실행 환경 복지몰, 제휴 가맹점, 도서 구입, 문화 활동 등에 사용할 수 있습니다.",
        complaint="답변이 너무 짧아요. 좀 더 친절하게 풀어서 설명해주면 안 되나요?",
        chunks=[
            "복지포인트는 실행 환경 복지몰과 제휴 가맹점에서 사용할 수 있다.",
    ...
    dict(
        ...
        pre_queries=["실행 환경 VPN 접속이 안 되는데요.", "MFA 기기를 바꿨는데 VPN 인증이 계속 실패해요."],
        answer="VPN 접속 문제는 대부분 네트워크 설정 때문입니다. 클라이언트를 재설치해 보세요.",
        complaint="MFA 기기 변경했을 때 재등록 절차를 알려달라니까요.",
        chunks=[
            "VPN 클라이언트 설치 및 초기 설정 안내: 실행 환경 포털에서 설치 파일을 내려받아 실행한다.",
            "VPN 접속 오류 시 네트워크 어댑터를 재설정하고 클라이언트를 재시작한다.",
            "MFA는 전사 필수 정책이며 모든 실행 환경 시스템 접근에 적용된다.",
        ],
    ),
    dict(
        ...
        context_dependent=True,
        pre_queries=["실행 환경 GPU 서버 사용 신청 어떻게 하나요?", "신청하면 얼마나 쓸 수 있어요?"],
        ...
        chunks=[
            ...
            "신청은 실행 환경 인프라 포털에서 진행한다.",
        ],
    ),
    dict(
        user="u3", conv="C-3002", turn=5, trap="distractor", expect=["rag_insufficient"],
        pre_queries=["실행 환경 계정 비밀번호 정책이 어떻게 되나요?", "비밀번호 변경 주기가 몇 개월인가요?"],
    ...
    dict(
        ...
        pre_queries=["HTTPS 기본 포트가 몇 번인가요?", "실행 환경 서버 방화벽에서 열려 있는 포트요."],
        answer="HTTPS는 443 포트를 사용합니다.",
        complaint="일반론 말고, 실행 환경 방화벽에서 실제로 허용된 포트 목록을 알려주세요.",
        chunks=[
    ...
    dict(
        ...
        context_dependent=True,
        pre_queries=["실행 환경 메일 용량 초과 시 해결 방법 알려주세요.", "단계별로 정리해줄 수 있나요?"],
        answer="실행 환경 메일 시스템의 용량이 초과되는 경우에는 여러 가지 방법을 고려해 볼 수 있는데, "
