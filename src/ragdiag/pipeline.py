...
**경계는 Case 다.** 왼쪽(로그를 읽고 필터를 거는 부분)은 실행 환경에 이미
구현돼 있으므로 이 저장소의 것은 여기서 검증할 때만 쓴다. 오른쪽(판정과 출력)이
복사 대상이고, Case 를 만들어 넣을 수만 있으면 그대로 돈다.
...
def build_outcome(owners: list, results: list[TurnResult],
    ...
    쓰므로 실행 환경의 파서가 만든 객체를 그대로 넣어도 된다.
    """
...
#
# 실행 환경에는 이 단계가 이미 있다. 여기 것은 검증용이다.
# ---------------------------------------------------------------------------
...
def read_turn_list(path: str | Path) -> list[TurnRef]:
    ...
    JSONL 을 뱉는 경우가 흔하고, 그것 때문에 실행 환경에서 변환 스크립트를 하나 더
...
def select_turns(conv_path: str | Path, turns_path: str | Path,
                 history_turns: Optional[int] = None,
                 limit: Optional[int] = None) -> Selection:
    """**필터를 실행 환경에 두고** 고른 턴만 받아 Case 로 만든다.

    필터 로직은 실행 환경 것을 쓰고 여기 것은 검증용이다. 그런데 턴을 Case 로 엮는 일
