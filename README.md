...
- **턴 하나가 판정 전부를 끝까지 간다** (`run.mode: turn`, 기본). 느린 턴이 다른 턴을 막지 않고,
  끝난 턴부터 10초마다 결과 파일에 쌓여 중간에 끊어도 거기까지는 남는다. 절반이 끝난 뒤
  중앙값의 4배(최소 60초)를 넘겨도 안 끝나는 턴은 `[단계] 낙오` 로 닫고 넘어간다 — 재실행하면
  그 턴만 다시 묻는다. `run.mode: stage` 는 기능 하나씩 턴 전부를 도는 예전 방식.
