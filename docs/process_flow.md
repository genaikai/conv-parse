...
> 실행 환경에는 필터가 이미 구현돼 있다. 이 저장소의 `filters.py` 는 여기서 검증할 때만
...
> 아니다. 실행 환경에서는 결과를 가져올 수 없어 화면과 이 파일이 전부이므로, 되짚을 일이
> 잦아지면 실리는 목록을 늘려야 한다.
...
분명할 때만 `general_knowledge` 로 둔다. 실행 환경 챗봇에서는 후자가 훨씬 흔하고,
...
**요구는 무엇으로 읽나.** `unmet_need` 다. 다만 그 칸이 질문에 없는 항목(절차 · 서류 · 기한)을
덧붙였으면 그 항목은 빼고, 질문보다 좁거나 다른 것을 가리키면("미주 말고 유럽") 그 칸을
따른다 — 후속 발화로 요구가 바뀐 것이다. 덧붙임을 빼는 건 프롬프트만 믿지 않는다 — **질문이
한 가지를 물었으면(`question_multi_intent=false`) 코드가 `unmet_need` 의 첫 항목만 넘긴다**
(쉼표 · 가운뎃점 · "및" 경계, `features/sufficiency.narrow_need`). 생각을 끈 약한 모델은
"덧붙임을 빼라"는 지시를 전혀 안 따랐고(부풀림 18회 중 8회 → 코드로 17회), 좁혀진 요구는 언제나
첫 항목이라 잃는 게 없다. 결과 파일의 `unmet_need` 는 원문 그대로다. Step 1 은 좁혀진 요구를 `unmet_need` 에는 언제나
담지만 `resolved_question` 에는 원래 질문과 합쳐 담는 경향이 있어서(Haiku 실측 7/24),
질문만 주는 설계는 쓰지 않았다. 부풀림과 좁힘을 이 한 문단으로 같이 처리한다
(`docs/design/sufficiency_step.md`).
...
| `reason` | 뜻 |
|---|---|
| `too_short` | 인용이 12자 미만(문장부호 · 한자 병기 제외). 짧으면 아무 문서에나 우연히 들어맞아 검증을 무력화한다 — 문서 10~15개 × 500자에서 8자 조각은 7.9%, 12자는 2.7% 가 우연히 통과했고, 판정자가 실제로 낸 인용은 13자보다 짧은 적이 없었다 |
| `not_found` | 어느 청크와도 연속 일치 90% 미만. **지어낸 인용이 여기 걸린다** |

**어떻게 대조하나** — 공백 · 문장부호 · 한자 병기("연가(年暇)")를 지우고 비교한다. 인용을
줄 · 줄임표(`…`) · 문장 끝 · 펴진 표의 행 경계(`| |`)에서 나눠 **조각마다** 대조하고, 조각
하나라도 없으면 전체가 떨어진다 — 표의 머리행 + 필요한 한 행, "A … B", 문장 + 표를 한 줄로
편 인용이 그래서 산다. 표 조각은 셀로 더 나누고 짧은 셀도 글자 그대로 있어야 한다 — 표를
세로로 잘라 "머리 셀 | 값 셀" 로 인용한 것이 살고, 값만 바꾼 행은 떨어진다. 조각이 원문의
어느 자리와 **한 글자 차이**면 통과하되, **숫자가 하나라도 다르면** 비율과 무관하게 떨어진다 —
규정문에서 숫자는 알맹이라 "100,000원" 을 "120,000원" 으로 바꾼 인용을 오탈자로 봐주면 안 된다.
전부 골든셋 변형 실측으로 정했다(`tests/test_citation_robustness.py`): 오탈자 1자 0% → 100%,
표 행 · 줄임표 · 세로 자르기 0% → 100%, 문장부호 40% → 100% 이 되는 동안 재서술 3% ·
지어낸 문장 0% · 숫자 한 자리 바꿈 1% · 다른 청크의 8~12자 조각 4% 다. 블록 합(흩어진 일치를
더하는 방식)은 재서술을 93% 통과시켜 쓰지 않는다.
...
| 주는 것 | 안 주는 것 |
|---|---|
| `resolved_question` — ⑤가 정리한 질문 | `unmet_need` |
| `llm_ans_on_last_q` — 비판받은 답변 | `current_query` — 불만 원문 |
| `rag_chunks` — 그 답변에 주어졌던 문서 | ⑦의 `verdict` · `evidence` |

**왜 질문을 주나.** 처음엔 감췄다 — 질문을 주면 "질문에 잘 답했나"라는 다른 판단이 섞여 ⑦과
같은 방향으로 쏠린 두 표가 된다는 이유였다. 그런데 질문이 없으면 이 단계는 "답변이 **아무**
청크나 썼는가"가 된다. 문서에 기한과 금액이 있고 사용자는 기한을 물었는데 답변이 금액만
말했다면, 답변은 문서를 썼지만 요구는 무시한 것이다 — case22 가 맞는데 `used` 로 읽혀 case17/13
으로 간다. 골든셋(문서 10~15개 × 500자, 42건 × Haiku 3회)에서 그런 15건 중 13건이 그렇게 샜다.
질문을 주고 "어느 청크가 관련 있는지 보는 데만 써라, 답변 품질을 평가하는 게 아니다"라고 못 박으면
15건이 전부 맞고 다른 범주(used · contradicted · 회피성 안내 · 함정)는 그대로다 — 42/42.
쏠림을 막는 장치는 질문을 감추는 것이 아니라 **불만 원문과 ⑦의 결과를 감추는 것**이다.
⑦이 인용한 청크만 주는 안(질문 없이)도 15건을 잡지만 답변이 문서를 쓴 케이스를 `ignored` 로
흘려 40/42 였다 (`docs/design/grounding_step.md`).
...
이 도구의 핵심이 거기라 전개해 둔다. 아래는 `features/route/table.py` 를 실제로 돌려 만든 것이다.
