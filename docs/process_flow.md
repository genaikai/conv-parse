...
`case0`(정상)과 `case30`(생성 붕괴)이 붙어 실제로 나올 수 있는 라벨은 **26종**이다. 우리가
더한 case 는 번호를 뒤에 받지만 **자리는 제 type 안이다** — 목록은 type 이 먼저, 그 안에서
번호순이다 (`taxonomy.sort_key`). 고르는 자리와 표가 같은 순서여야 표에서 본 case 를 찾을 수 있다. `✗` 다섯은 로그에 필드가
...
판정 기능(④~⑩)은 두 방식으로 돌 수 있다 (`run.mode`).

| | `turn` (기본) | `stage` |
|---|---|---|
| 순서 | 턴 하나가 ④~⑩ 을 끝까지 간다. 워커 수만큼 동시에 | 기능 하나가 턴 전부를 돈 뒤 다음 기능 |
| 느린 턴 | 자기 워커만 붙잡는다. 다른 턴은 끝까지 간다 | 그 단계가 끝나지 못해 다음 단계가 기다린다 |
| 낙오자 | 턴 전체 소요 시간이 기준 (그 턴이 머무는 기능 이름으로 닫힘) | 그 단계 소요 시간이 기준 |
| 중간 결과 | 끝난 턴부터 10초마다 결과 파일에 쌓인다 | 전부 끝난 뒤 한 번 |
| 진행 표시 | `[판정] 128/540` 한 줄 | `[관측]` `[충족도]` `[근거 활용]` 단계마다 |

낙오자 기준은 같다 — 절반(8건 이상)이 끝난 뒤, 중앙값 × `run.straggler_factor`(4)와
`run.straggler_min_sec`(60) 중 큰 쪽. 넘겨도 안 오는 턴은 `[단계] 낙오 …` 로 닫히고 늦은 응답은
버려진다. 재실행하면 성공한 호출은 캐시에서 나오고 그 턴만 다시 묻는다. 집계(classification ·
failures 등)는 어느 방식이든 판정이 전부 끝난 뒤 한 번 돈다. 두 방식의 판정 결과는 같다 —
`tests/test_run_mode.py` 가 스냅숏 시나리오 전부로 확인한다.

## 전체 그림

```
conv-data ─┐
           ├─▶ ① 파싱·짝짓기 ─▶ ② 필터 ─▶ ③ Case ─┬─▶ ④ 서비스오류 · 생성붕괴 규칙? 코드 ─▶ case9 · case30 (끝)
filter-data┘                                        ├─▶ ④′ 읽을 수 있나?  LLM  ← 답변만 봄 ─▶ 아니면 case30 (끝)
                                                    │
...
## ④′ 읽을 수 있는 글인가 — 답변만 보고 LLM 이 한 번

**목적** — 모델이 답을 만들다 무너진 턴(생성 붕괴)을 관측 전에 걸러낸다. `5555…` 처럼
오탐이 없는 모양은 ④의 코드 규칙(`degenerate`)이 잡고, 토큰 잡탕 · 중간부터 깨진 것 ·
문단이 조금씩 바뀌며 되풀이되는 것 · 시스템 프롬프트가 새어 나온 것은 규칙으로 열거할 수
없어 여기서 LLM 에 묻는다.

**입력** — `llm_ans_on_last_q` **하나.** 질문 · 불만 · 문서를 주지 않는다. 주면 "질문에
맞는 답인가" 를 재기 시작하고, 그러면 부실한 답변까지 붕괴로 세어 case20 · 22 가 사라진다.
묻는 것도 하나다 — 사람이 읽고 뜻을 잡을 수 있는가. 유용한지 · 맞는지는 묻지 않는다.

**출력** — `legible` · `quote`(깨진 구절, 원문에서 글자 그대로) · `reasoning`.
`legible=false` 이고 인용이 원문과 맞으면 `case30` · 신뢰도 **medium** 으로 끝난다(LLM 1회).
인용이 안 맞으면 판정은 무효이고 ⑤로 흘러간다 — 이 판정이 틀리면 그 턴의 진짜 원인은
아무 단계에서도 판정받지 못하므로, 확신이 없으면 legible 이라고 프롬프트에 못 박았다.
코드 규칙이 잡은 case30 은 high, 여기서 잡은 것은 medium 이라 대시보드에서 색으로 갈린다.

설정 `run.legibility: false` 로 끈다. 그러면 ④의 코드 규칙만 남고 호출이 턴당 하나 준다.
...
|---|---|
| 보통의 턴 | 19개 전부 |
| `case9` · `case30` (④에서 코드로 끊김) | **없음.** LLM 을 한 번도 안 불렀다 |
| `case30` (④′에서 읽기 판정으로 끊김) | **없음.** `evidence.legibility` 만 있다 |
| 판정 실패(`error`) | 없음 |
...
| `degenerate` | 답변이 비어 있음 | — | 공백을 지운 답변이 10자 미만이면 ok(판단 안 함). 서로 다른 글자가 2종 이하 · 1~30자 조각이 **글자 그대로 연달아** 4번 이상 되풀이되어 90% 이상(다른 내용을 사이에 둔 되풀이는 구조라 안 잡음) · 한글/영문/한자/숫자가 하나도 없음 · 끝에 같은 글자 50개 이상(마크다운 구분선 글자 제외) 중 하나면 violated |
...
| `case30` | 생성 붕괴 | high | 코드 (④ 규칙). ④′ 읽기가 잡으면 medium |
...
| 호출 | 어디까지 |
|---|---|
| 0 | ④에서 끊김 (case9 · case30) 또는 캐시 히트 |
| 1 | ④′에서 끊김 (읽을 수 없는 답변 · case30) |
| 2 | ④′+⑤. 형식·언어·길이 불만이나 도메인이 아닌 질문 |
| 3 | ④′+⑤+⑦. 문서가 불충분해 ⑨까지 안 감 |
| 4 | ④′+⑤+⑦+⑨. 문서는 충분했고 활용 여부까지 물음 |
