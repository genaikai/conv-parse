...
나머지는 이렇게 도는 프로젝트라면 어디나 해야 하는 것들이다 — 실값을 채우고,
...
미리 만들어져 있다. `← 채우세요` 가 붙은 **네 줄**만 채우면 된다.
...
나머지 키는 전부 기본값으로 돈다. 경로(`conv_data` 등)는 실행할 때
...
**작성 규칙: [`todo/filter.md`](todo/filter.md)**
...
**작성 규칙: [`todo/scripting.md`](todo/scripting.md)**
...
**이 프로젝트만의 것이 남아 있다.** `configs/env.yaml`·`outputs/`·`notebooks/` 는
자동으로 무시 목록에 들어가지만, 아래 둘은 이 프로그램의 사정이라 직접 넣어야 한다.

| | 무엇이 들어 있나 | 커밋해도 되나 |
|---|---|---|
| `.cache/` | **LLM 판정 응답.** 실데이터에서 뽑은 관측·인용이 그대로 | ⚠ 직접 넣어야 한다 |
| `data/` | 실데이터 | ⚠ 직접 넣어야 한다 |
| `output/` | 분류 결과 · RUN SUMMARY | 남기고 싶을 수 있다 — 판단 |
...
`작업 폴더 안의 사본` 사본이 커밋되는 것은 **목적이다** — 결과를 가져올 수 없는 상황에서
...
결과 파일도 플롯도 로그도 가져올 수 없다. 돌아오는 것은 **화면을 보고 옮겨 적은 것**
